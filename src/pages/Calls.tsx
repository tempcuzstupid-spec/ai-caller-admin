// Calls page — multi-tenant + multi-vertical aware.
//
// Each call is scoped to the tenant. The agentConfig the user picks is one of
// the tenant's deployed agentConfigs (tenant_id filtered at the tRPC layer).
// The "placeCall" mutation routes through the FastAPI AI Caller backend in
// production; in this admin it's a tRPC endpoint that wraps Twilio's REST API.
...
export default function Calls() {
  ...
  const transcript = trpc.calls.transcript.useQuery(
    { callId: viewCallId! },
    { enabled: viewCallId !== null }
  );
  ...
  const verticalById = (id: number) => verticals.data?.find((v) => v.id === id);
  const outboundAgents = (agents.data ?? []).filter(
    (a) => verticalById(a.verticalId)?.direction !== "inbound"
  );
  ...
  return (
    <AuthLayout>
      <div className="p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid md:grid-cols-4 gap-3 items-end">
            ...
            <Button
              disabled={!agentConfigId || !to || place.isPending}
              onClick={() =>
                place.mutate({
                  agentConfigId: Number(agentConfigId),
                  to,
            ...
            >
              ...
              {place.isPending ? "Dialing…" : "Call now"}
            </Button>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {list.data?.map((c) => (
                <div
                  key={c.id}
                  className="flex items-center justify-between gap-3 border-b last:border-0 pb-2 text-sm"
                >
      ...
      <Dialog
        ...
        onOpenChange={(o) => !o && setViewCallId(null)}
      >
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {transcript.data?.lines.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No transcript recorded. Transcripts arrive from your voice gateway
              when the call ends.
            </p>
          )}
          <div className="space-y-3">
            {transcript.data?.lines.map((l) => (
              <div
                key={l.id}
                className={l.role === "assistant" ? "text-left" : "text-right"}
              >
                <div
